...
export function calculateInitialPrice(input: CalculationInput): CalculationResult {
    ...
    // Harga minimum dan maksimum yang diperbolehkan
    const MIN_PRICE = 99;
    const MAX_PRICE = 150000000; // 150 juta

    // Binary search untuk menemukan harga awal yang tepat
    // Untuk net price kecil/negatif, harga awal tetap harus >= MIN_PRICE
    let low = MIN_PRICE;
    let high = Math.min(Math.max(MIN_PRICE, desiredNetPrice * 3, 10000), MAX_PRICE);
    ...
    // Dengan batasan minimum MIN_PRICE
    ...
    while (finalInitialPrice > MIN_PRICE) {
        ...
        for (const fee of breakdown) {
            if (fee.isApplied) totalFee += calculateFeeAmount(fee, finalInitialPrice - 1);
        }
    ...
    return {
        initialPrice: Math.min(finalInitialPrice, MAX_PRICE),
...
// Fungsi untuk menghitung harga bersih dari harga awal (reverse calculation)
export function calculateNetPrice(
    input: CalculationInput & { initialPrice: number },
): CalculationResult {
    const {
        sellerType,
        category,
    ...
    // 9. Biaya Proses Pesanan (flat fee)
    breakdown.push({
        name: "Biaya Proses Pesanan",
        percentage: 0,
        amount: ORDER_PROCESSING_FEE,
    ...
    for (const fee of breakdown) {
        ...
        if (fee.percentage === 0) {
            ...
            let feeAmount: number;
            if (fee.name === "Premi") feeAmount = Math.ceil((fee.percentage / 100) * initialPrice);
            else feeAmount = Math.round((fee.percentage / 100) * initialPrice);
    ...
    const netPrice = initialPrice - totalFee;
    const effectiveFeePercentage = (totalFee / initialPrice) * 100;
